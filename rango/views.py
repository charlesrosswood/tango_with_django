...
def analyse_mturk(request):
	...
	if request.method == 'POST':
		...
		if csv_form.is_valid(): # method to check whether all the fields are correct
			...
			graph_types_1 = list(set(analysed_tags['mcc to high level tags'].keys() + analysed_tags['mcc to low level tags'].keys()))
			graph_types_2 = list(set(analysed_tags['high level tags to mcc'].keys() + analysed_tags['low level tags to mcc'].keys()))
			
			# print graph_types

			context_dict = {
				'graph': graph,
				'graph_types_1':sorted(graph_types_1),# this is the categories tags
				'dataset_options_1': dataset_options_1, # this is the high or low level tags
				'graph_types_2':sorted(graph_types_2),# this is the categories tags
				'dataset_options_2': dataset_options_2 # this is the high or low level tags
			}
...
def RBS_transactions_analysis(request):
	...
	if request.method == 'POST':
		...
		if csv_form.is_valid(): # method to check whether all the fields are correct
			...
			file_uploaded = True
			# print 'csv_file.csv_file is', csv_file.csv_file
			analysed_txns = analysis.read_large_transactions_csv( csv_file )
			graph = json.dumps(analysed_txns)
			dataset_options = ['spends']
			template = 'rango/d3_histograms.html'
